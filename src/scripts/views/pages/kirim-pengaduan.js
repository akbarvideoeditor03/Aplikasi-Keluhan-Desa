...
const kirimPengaduan = {
  async render() {
    ...
    const kepalaDesaOptions = kepalaDesa.length > 0
      ? kepalaDesa.map((kd) => `<option value="${kd.id}">${kd.nama} | Kepala Desa ${kd.desa}, kecamatan ${kd.kecamatan}</option>`).join('')
      : '<option disabled>Tidak ada kepala desa yang tersedia</option>';

    return `
      ${user && user.role == 'pengguna' ? `
        <div class="content">
          <h2>Kirim Pengaduan <i class="bi bi-envelope-arrow-up-fill"></i></h2>
          <div id="menu" class="menu">
            <div class="form">
              <form id="pengaduanForm">
                <h4 class="content__subheading">Form Pengaduan</h4>
                <p><i class="bi bi-exclamation-triangle-fill text-yellow"></i> mohon isi form dengan benar dan dapat dipertanggung jawabkan</p>
      ...
      ` : `
        ${user && user.role == 'kepala desa' || user && user.role == 'admin' ? `
          <div class="container col-container center">
            <p style="font-size: large;"><i class="bi bi-emoji-frown-fill"></i> Maaf... fitur ini hanya diperuntukan untuk pengguna umum</p>
          </div>
        `:`
          <div class="container col-container center">
